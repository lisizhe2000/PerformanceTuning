...
### 1. 实现与FLASH的公平比较 —— 不使用SAT求解器生成解，而是直接从csv文件中采样
    ...
    3. 在采集函数中引入方差，或取其上界
        1. 随机森林     √
        2. 各种集成学习
        3. 多个异构模型，取最大预测值   # ! TODO   （批注：我认为不可行。找不到最优配置的关键不在于模型。模型已经足够精确了，但是是由于性能的测量误差，导致性能不完全遵循模型的描述。关键在于搜索过程中的扰动）
...
### 实验观察：无法找到最优值的原因
    ...
    1. 与整体错误率无关
    2. 与前3%优秀解的错误率无关
    ———— 1. 最优解附近的配置没有被采样，导致这部分解的性能始终被低估
    ———— 2. 其他位置的解性能始终被高估，导致一直采样其他解（有时候前0.5%的错误率不低，但仍能找到最优解）

    最终结果与n_pred_better_than_best有显著的正相关性

# 2023.11.20

1. 再用其他集成学习方法做一下。xgboost, adaboost, random forest...
2. 用JavaGC来做，跟flash对比一定有优势。（flash遍历太耗时）难点在于数据处理
3. JavaGC load balancing? 看起来是个可以努力的方向